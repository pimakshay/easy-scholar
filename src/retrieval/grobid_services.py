...
class GrobidDocumentParser(BaseBlobParser):
    ...
    def __init__(
        ...
        process_header_document: str = "api/processHeaderDocument",
        process_full_document: str = "api/processFulltextDocument",
        process_references: str = "api/processReferences",
        process_headers: str = "api/processHeaderNames",
    ) -> None:
        self.segment_sentences = segment_sentences
        self.grobid_server = grobid_server
        self.process_header_document = process_header_document
        self.process_full_document = process_full_document
        self.process_references = process_references
        self.process_headers = process_headers
        try:
    ...
    def get_document_intro(
        ...
        document_content_description = f"Paper Title: {paper_title}; Abstract: {abstract}; Keywords: {keywords}"
        paragraph_dict = {
            "text": "".join(document_content_description),
            "para": str(0),
            "bboxes": [],
    ...
    def process_xml(
        ...
        chunks.append(self.get_document_intro(soup))
    ...
    def lazy_parse(self, blob:Blob) -> Iterator[Document]:
        ...
        try:
            ...
            r = requests.request(
                "POST",
                self.grobid_server + self.process_full_document,
                headers=None,
                params=None,
                files=files,
                data=data,
    ...
    def parse_references(self, file_path:str) -> Iterator[Document]:
        if file_path is None:
            raise ValueError("blob.source cannot be None.")
        pdf = open(file_path, "rb")
        files = {"input": (file_path, pdf, "application/pdf", {"Expires": "0"})}
        try:
            data: Dict[str, Union[str, List[str]]] = {}
            for param in ["consolidateCitations", "includeRawCitations"]:
                data[param] = "1"
            files = files or {}
            r = requests.request(
                "POST",
                self.grobid_server + self.process_references,
                headers=None,
                params=None,
                files=files,
                data=data,
        ...
        if xml_data is None:
            ...
            return r, grobid_tei_xml.parse_citations_xml(xml_data)
